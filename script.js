...
function displayTippingForm() {
	...
	roundRef.get().then(function(doc) {
		if (doc.exists) {
			...
			htmlTitle = "<select class='roundSelector'>";
			var i;
			var roundCount = 23;
		...
		roundRef.collection("fixtures").orderBy("date")
		  ...
		  .then(function(docs) {
			...
			docs.forEach(function(doc) {
				...
				htmlFields = htmlFields + "<div class='flags'><div class='flag flag-" + i + "' id='NA'></div></div><div class='inputs'><span>&#9660;</span><select class='formInput' id='clubInput-" + i + "'><option disabled selected value style='display: none;'></option><option id='home-" + i + "' value='" + homeTeam + "'>" + homeTeamLong + "</option><option id='away-" + i + "' value='" + awayTeam + "'>" + awayTeamLong + "</option><option id='draw-" + i + "' value='DRW'>Draw</option></select><input type='number' min='0' max='200' class='formInput' id='marginInput-" + i + "' value='0'></input></div>";
